...
Para implementar os serviços do **Banking as a Service (BaaS)**, é necessário que o parceiro configure uma URL para o recebimento dos eventos de callback. 
...
Após configurar a URL de callback (homologação e produção), escolher qual método HTTPS e qual o método de autenticação devem ser utilizados, o parceiro deve informar o time de integração da BMP, para que essa configuração seja parametrizada na BMP.
...
O parceiro deve escolher um método HTTPS que gostaria de utilizar para recebimento dos callbacks. 
