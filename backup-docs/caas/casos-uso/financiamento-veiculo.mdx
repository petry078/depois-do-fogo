...
O primeiro passo para a contratação de crédito na modalidade Financiamento de Veículos é a simulação.
...
Após a simulação e o onboarding, é necessário realizar a requisição de contratação do crédito. Chamamos essa etapa de **Inclusão de Proposta**.
...
<Note>O campo `nroDiasAcrescimo` define quantos dias serão adicionados ou subtraídos da data de vencimento padrão da primeira parcela. Por padrão, o vencimento ocorre no mesmo dia da simulação, mas no mês seguinte. Exemplo: se a simulação for em 27/05, o vencimento padrão será 27/06. Se você preencher com "20", o vencimento será 20 dias após essa data padrão (ex: 17/07).</Note>
...
Após a requisição de contratação, é necessário realizar a inclusão dos dados do veículo.

<Expandable title="Inclusão de Dados do Veículo">

Utilize o endpoint `/Proposta/IncluirDadosVeiculos` para incluir dados do veículo. 

Para mais informações sobre este endpoint, acesse o documento de referência [34 - Registro de Veículo](https://bmpdocs.moneyp.com.br/caas/referencias-de-api/garantias/34-registro-de-veiculo).
...
Após informar os dados do veículo, é necessário coletar as assinaturas dos envolvidos na proposta.
...
Este é o último passo do processo de contratação do crédito na modalidade Financiamento de Veículos.
...
<Warning>Em casos de utilização do gravame automático, não é necessário chamar o endpoint de liberação de proposta. O processo ocorre automaticamente conforme a vinculação do gravame ao financiamento.</Warning>
...
* [33 - Cancelamento de Contrato](https://bmpdocs.moneyp.com.br/caas/referencias-de-api/gestao-de-propostas/33-cancelamento-de-contrato): utilizado para cancelamento de contrato;
