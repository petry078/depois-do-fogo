...
Esse método envolve a geração de tokens que permitem o acesso seguro aos nossos servidores, softwares e APIs. 

A autenticação através do OAuth 2.0 ajuda a prevenir fraudes cibernéticas e o vazamento de informações confidenciais, utilizando criptografia de chave pública para validar as identidades.
...
curl --location 'https://auth.moneyp.dev.br/connect/token' \
  ...
  --data-urlencode 'scope=econsignado.api bmp.digital.api.full.access' \
  --data-urlencode 'client_assertion=<Token JWT>' \ // JWT gerado no passo anterior.
  --data-urlencode 'client_assertion_type=urn:ietf:params:oauth:client-assertion-type:jwt-bearer'
```

<Note>Substitua `<Token JWT>` pelo token gerado no passo anterior.</Note>
